...
template <class T>
bool GeodesicEdge<T>::operator==(const GeodesicEdge &r)
{
...
template <class T>
void GeodesicGrid<T>::subdivide(int levels)
{
...
//
// add a new cell, and check for duplicate verts within a threshold distance
//
template <class T>
GeodesicCell<T> *GeodesicGrid<T>::add_unique_cell_at_point(Float3 &p, GeodesicCell<T> *_cells, int &cell_count, GeodesicCell<T> *a, GeodesicCell<T> *b)
{
...
//
// add a new cell, and we DGAF about duplicates
//
template <class T>
GeodesicCell<T> *GeodesicGrid<T>::add_cell_at_point(Float3 &p, GeodesicCell<T> *_cells, int &cell_count)
{
  _cells[cell_count].init();
  _cells[cell_count].pos = p;
  _cells[cell_count].color = Float3(1.0f, 0.0f, 0.0f);
...
template <class T>
void GeodesicGrid<T>::select_next_cell()
{
  selected_cell++;
  ...
  cout<<"selected_cell: "<<selected_cell<<endl;
}

template <class T>
void GeodesicGrid<T>::select_prev_cell()
{
  selected_cell--;
  if(selected_cell < 0)
  {
...
template <class T>
void GeodesicGrid<T>::make_selected_neighbors_clockwise()
{
...
template <class T>
GeodesicCell<T> *GeodesicGrid<T>::get_cell_by_uv(Float2 uv, GeodesicCell<T> *start)
{
  //
  // greedy walk
  //
